...
stockPriceAnalyst = Agent(
    ...
    backstory="You are a highly experienced in analyzing the price of an especific stock and make predictions about its future price",
    verbose=True,
    llm= llm,
    max_iter=5,
    memory=True,
...
newsAnalyst = Agent(
    ...
    goal="Create a short sumary of the market news related to the stock {ticket} company. Specify the current trend - up, down or sideways with the news context. For each requets stock asset, specify a number between 0 and 100, where 0 is extreme fear and 100 is extreme greed.",
...
with st.sidebar:
    ...
    with st.form(keys='research_form'):
